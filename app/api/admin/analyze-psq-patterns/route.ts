...
interface PSQPattern {
  pattern: string // e.g., "donate", "donate-page"
  identifier: string // e.g., "greg-abbott"
  fullUrl: string
  count: number
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[Admin] Starting PSQ Impact pattern analysis...")
    ...
    console.log(`[Admin] Found ${emailCampaigns.length} email campaigns and ${smsMessages.length} SMS messages to scan`)

    // Extract all PSQ URLs and their patterns
    const patternMap = new Map<string, PSQPattern>()
    ...
    const processLinks = (links: any) => {
      ...
      for (const link of linksArray) {
        ...
        try {
          ...
          // Check if it's a PSQ Impact URL
          if (urlObj.hostname.includes("psqimpact.com")) {
            ...
            if (pathParts.length > 0) {
              ...
              // Example: https://secure.psqimpact.com/donate/greg-abbott/launch-goal-dec8
              if (pathParts[0] === "donate" && pathParts.length > 1) {
                pattern = "donate"
                identifier = pathParts[1]
              } else if (pathParts[0] === "donate-page" && pathParts.length > 1) {
                pattern = "donate-page"
                identifier = pathParts[1]
    ...
    console.log(`[Admin] Found ${patterns.length} unique PSQ Impact URL patterns`)
    ...
    console.error("[Admin] Error analyzing PSQ Impact patterns:", error)
    return NextResponse.json({ error: "Failed to analyze PSQ Impact patterns" }, { status: 500 })
  }
}

...
export async function POST(req: Request) {
  ...
  try {
    // Find all unassigned campaigns from American Liberty Muse
    const unassignedCampaigns = await prisma.competitiveInsightCampaign.findMany({
      where: {
        entityId: null,
        senderEmail: {
          contains: "americanlibertymuse.com",
        },
      },
      select: {
        id: true,
    ...
    // Get American Liberty Muse entity
    ...
    // Get entities with donation identifiers
    ...
    for (const campaign of unassignedCampaigns) {
      ...
      const containsAmericanLibertyDomain = urls.some(
        (url) => url.includes("americanliberty.news") || url.includes("www.americanliberty.news"),
      )

      // Rule 1: If 9+ links AND contains americanliberty.news domain → Assign to American Liberty Muse
      if (linkCount >= 9 && containsAmericanLibertyDomain) {
        if (americanLibertyMuse) {
          ...
          results.assignments.push({
            ...
            assignedTo: americanLibertyMuse.name,

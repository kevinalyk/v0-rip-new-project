...
export async function POST(req: Request) {
  ...
  try {
    console.log("[MAGA Daily Updates Assignment] Starting assignment process...")

    // Get all unassigned campaigns from MAGA Daily Updates
    const unassignedCampaigns = await prisma.competitiveInsightCampaign.findMany({
      where: {
        entityId: null,
        senderEmail: "updates@emails.magadailyupdates.com",
      },
      select: {
        id: true,
        subject: true,
    ...
    console.log(
      `[MAGA Daily Updates Assignment] Found ${unassignedCampaigns.length} unassigned campaigns from MAGA Daily Updates`,
    )
    ...
    for (const campaign of unassignedCampaigns) {
      try {
        ...
        console.log(
          `[MAGA Daily Updates Assignment] Campaign ${campaign.id}: Found ${winredIdentifiers.size} WinRed identifiers`,
        )
        ...
        if (matchedEntity) {
          ...
          console.log(
            `[MAGA Daily Updates Assignment] Assigned campaign ${campaign.id} to ${matchedEntity.entity.name} via donation identifier`,
          )
        } else {
          ...
          console.log(`[MAGA Daily Updates Assignment] Skipped campaign ${campaign.id} - no matching entity found`)
        }
      } catch (error) {
        console.error(`[MAGA Daily Updates Assignment] Error processing campaign ${campaign.id}:`, error)
        skippedCount++
        results.push({
          campaignId: campaign.id,
          subject: campaign.subject || "No subject",
    ...
    console.log(
      `[MAGA Daily Updates Assignment] Assignment complete: ${assignedCount} assigned, ${skippedCount} skipped`,
    )
    ...
    console.error("[MAGA Daily Updates Assignment] Error:", error)
    return NextResponse.json(
      {
        error: "Failed to assign MAGA Daily Updates campaigns",

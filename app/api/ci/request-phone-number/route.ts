...
export async function POST(request: NextRequest) {
  try {
    ...
    // Free accounts (no active paid subscription) cannot request personal phone numbers —
    // this is a paid add-on available on Basic and up.
    if (!client || client.subscriptionPlan === "free") {
      return NextResponse.json(
        { error: "Personal phone numbers require a paid plan. Please upgrade to Basic, Professional, or Enterprise." },
        { status: 403 },
      )
    }

...
export async function GET(
  ...
  try {
    // Check for pre-generated Blob URL first — instant, no cold start
    const campaign = await prisma.competitiveInsightCampaign.findUnique({
      where: { shareToken: token },
      select: { emailContent: true, emailPreview: true, ogImageUrl: true },
    })
    ...
    const smsCached = await prisma.smsQueue.findUnique({
      where: { shareToken: token },
      select: { message: true, ogImageUrl: true },
    })
    ...
    // Not yet pre-generated — fall back to live Puppeteer screenshot
    let emailHtml = ""
    let isSms = false
    let smsMessage = ""

...
| Repo-wide `pnpm run lint` | 1 | 21677 pre-existing problems, none in any mobile file or `app/login/**` (confirmed by grepping the output for `mobile`) |
| Targeted `eslint` on `app/login/page.tsx` / `app/login/loading.tsx` (Goal 4 `dvh` fix, this revision) | 1 | 3 pre-existing errors in `page.tsx` (unrelated lines: unused `err`, `any`, unescaped apostrophe), 0 in `loading.tsx`; none on the lines this revision changed |
| `pnpm run build` (rerun after the `dvh` fix, dev DB) | 0 | `✓ Compiled successfully`; `/login` present at 5.1 kB in the route manifest |

This revision's only code change is adding the `min-h-dvh` fallback (alongside the
existing `min-h-screen`) to the three viewport-filling containers in
`app/login/page.tsx` and `app/login/loading.tsx`, matching the pattern already used in
`app-layout.tsx`, to close a Goal 4 gap where the login shell didn't get the same
`dvh` treatment as the rest of the app. The repo-wide `tsc --noEmit` row (569
pre-existing errors, one landing on an unchanged context line in
`components/sidebar.tsx` — see above) from the prior revision of this table was not
rerun, since this revision's change is two Tailwind class-list edits with no type
surface. Repo-wide `pnpm run lint` was rerun in full for this revision (see row
above) and confirmed to contain zero errors in either changed file or in any
`app/api/mobile/**` file.

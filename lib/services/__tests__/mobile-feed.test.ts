...
async function main() {
  ...
  const clientA = await prisma.client.create({
    data: { id: `${PREFIX}client_a`, name: `${PREFIX}Client A`, slug: `${PREFIX.toLowerCase()}client-a`, active: true, subscriptionPlan: "all" as any, dataRetentionDays: 90 },
  })
  const clientB = await prisma.client.create({
    data: { id: `${PREFIX}client_b`, name: `${PREFIX}Client B`, slug: `${PREFIX.toLowerCase()}client-b`, active: true, subscriptionPlan: "all" as any, dataRetentionDays: 90 },
  })
  ...
  try {
    ...
    await test("feed listing includes shared (entity-assigned) campaigns for any client", async () => {
      const { items } = await getFeedPage(clientB.id, "all" as any, {}, null)
    ...
    await test("feed listing never includes a client's own personal (unassigned) capture", async () => {
      const { items } = await getFeedPage(clientA.id, "all" as any, {}, null)
      assert(
    ...
    await test("getFeedItemById: a client's own personal capture is visible to itself only", async () => {
      const ownView = await getFeedItemById(clientA.id, "all" as any, personalCampaignA.id, "email")
      assert(ownView !== null, "owning client should see its own personal capture")
      const otherView = await getFeedItemById(clientB.id, "all" as any, personalCampaignA.id, "email")
    ...
    await test("getFeedItemById: shared campaigns are visible to any client", async () => {
      const view = await getFeedItemById(clientB.id, "all" as any, sharedCampaign.id, "email")
    ...
    await test("getFeedItemById: a data_broker-assigned campaign is not visible to a non-owning client", async () => {
      const view = await getFeedItemById(clientA.id, "all" as any, dataBrokerCampaign.id, "email")
    ...
    await test("subscriptionsOnly=true with zero follows returns an empty feed, not the full feed", async () => {
      const { items } = await getFeedPage(clientB.id, "all" as any, { subscriptionsOnly: true }, null)
    ...
    await test("tag filter is scoped per-client and restricts the feed to tagged entities", async () => {
      ...
      try {
        const tagged = await getFeedPage(clientA.id, "all" as any, { tag: "watchlist" }, null)
        assert(tagged.items.some((i) => i.id === sharedCampaign.id), "entity tagged 'watchlist' should appear")

        const untagged = await getFeedPage(clientB.id, "all" as any, { tag: "watchlist" }, null)
    ...
    await test("search filter matches email subject/sender", async () => {
      const { items } = await getFeedPage(clientA.id, "all" as any, { search: "Shared campaign" }, null)
      assert(items.some((i) => i.id === sharedCampaign.id), "search should match the campaign subject")
      const { items: noMatch } = await getFeedPage(clientA.id, "all" as any, { search: "nonexistent-search-term-xyz" }, null)
    ...
    await test("search filter matches sms message content", async () => {
      const { items } = await getFeedPage(clientA.id, "all" as any, { search: `${PREFIX}Please donate`, messageType: "sms" }, null)
    ...
    await test("unprocessed SMS is excluded from the feed listing", async () => {
      const { items } = await getFeedPage(clientA.id, "all" as any, { messageType: "sms" }, null)
    ...
    await test("getFeedItemById returns null for unprocessed SMS", async () => {
      const view = await getFeedItemById(clientA.id, "all" as any, unprocessedSms.id, "sms")
    ...
    await test("getFeedItemById returns processed SMS detail", async () => {
      const view = await getFeedItemById(clientA.id, "all" as any, smsCampaign.id, "sms")
    ...
    await test("Client.dataRetentionDays hides items older than the retention window even on an unlimited-history plan", async () => {
      ...
      try {
        const { items } = await getFeedPage(clientTightRetention.id, "all" as any, {}, null)
        assert(
        ...
        const view = await getFeedItemById(clientTightRetention.id, "all" as any, oldCampaign.id, "email")
    ...
    await test("cursor pagination returns a distinct, complete second page", async () => {
      ...
      try {
        const page1 = await getFeedPage(clientA.id, "all" as any, {}, null)
        ...
        const page2 = await getFeedPage(clientA.id, "all" as any, {}, cursor)

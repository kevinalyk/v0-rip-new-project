...
export const SUBJECT_PATTERNS = {
  ...
  personalization: {
    ...
    description: "Contains a merge tag like {{Name}}, [NAME], or {first_name}",
  },
  number_dollar: {
    label: "Number / Dollar",
    description: "Contains a dollar amount, digit, or spelled-out match amount",
  },
  urgency: {
...
// Merge tag patterns: {{Name}}, {first_name}, [NAME], [FIRST NAME], etc.
const PERSONALIZATION_REGEX =
  /\{\{[^}]+\}\}|\{[a-zA-Z_]+\}|\[[A-Z][A-Z _]+\]/

// Dollar amounts or numeric values: $5, $5,000, 5x, 3X, "five dollars"
const NUMBER_DOLLAR_REGEX =
  /\$[\d,]+|[\d,]+\$|\b\d+[xX]\b|\b\d+(?:,\d{3})*(?:\.\d+)?\b/

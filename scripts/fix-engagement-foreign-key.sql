...
-- This happens when the column types don't match exactly

-- First, let's check what type the SeedEmail.id column actually is
-- and make sure our foreign key column matches

-- Drop the existing table if it exists (since it has the wrong constraint)
DROP TABLE IF EXISTS "EngagementLog";
DROP TABLE IF EXISTS "SenderFamiliarity";

-- Recreate EngagementLog with proper foreign key reference
-- Using the same data type as SeedEmail.id (likely SERIAL/INTEGER)
CREATE TABLE "EngagementLog" (
    "id" SERIAL PRIMARY KEY,
    "seedEmailId" INTEGER NOT NULL,
...
-- Recreate SenderFamiliarity with proper foreign key reference
